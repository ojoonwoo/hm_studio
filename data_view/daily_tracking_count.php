							...
							<table class="table table-hover" style="position: relative;">
								<thead>
									<tr>
										...
										<th>tracking_medium</th>
										<th>tracking_campaign</th>
										<th>tracking_content</th>
										<th>PC</th>
										<th>Mobile</th>
										<th>Total</th>
									</tr>
...
//	print_r($daily_date_query);
	...
	while($date_daily_data = mysqli_fetch_array($date_res))
	{
		...
		$media_query	= "SELECT tracking_media, COUNT( tracking_media ) media_cnt FROM tracking_info WHERE 1 AND tracking_date LIKE  '%".$daily_date."%' GROUP BY tracking_source, tracking_medium, tracking_campaign, tracking_content";
		...
		while ($media_daily_data = mysqli_fetch_array($media_res))
		{
			...
			// $pc_query		= "SELECT * FROM tracking_info WHERE 1 AND tracking_date LIKE  '%".$daily_date."%' AND tracking_gubun='PC' GROUP BY tracking_source, tracking_medium, tracking_campaign, tracking_content";
			// $pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
			// $mobile_query	= "SELECT * FROM tracking_info WHERE 1 AND tracking_date LIKE  '%".$daily_date."%' AND tracking_gubun='MOBILE' GROUP BY tracking_source, tracking_medium, tracking_campaign, tracking_content";
