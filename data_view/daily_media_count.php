...
<?php
	$where = "";
	if ($sDate != "")
		$where	.= " AND mb_regdate BETWEEN '".$sDate." 00:00:00' AND '".$eDate." 23:59:59'";
							
	$pc_total_all = 0;
	$mobile_total_all = 0;
	$unique_total = 0;
	$all_total = 0;
	$daily_date_query	= "SELECT mb_regdate FROM member_info WHERE 1 ".$where." Group by substr(mb_regdate,1,10) ORDER BY mb_regdate DESC";
	$date_res			= mysqli_query($my_db, $daily_date_query);
	while($date_daily_data = mysqli_fetch_array($date_res))
	{
		$daily_date		= substr($date_daily_data['mb_regdate'],0,10);
		$media_query	= "SELECT utm_source, utm_medium, utm_campaign, utm_content, COUNT(*) media_cnt FROM member_info WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' GROUP BY utm_source, utm_medium, utm_campaign, utm_content";
		...
		{
			$pc_query		= "SELECT * FROM member_info WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' AND utm_source='".$media_data['utm_source']."' AND utm_medium='".$media_data['utm_medium']."' AND utm_campaign='".$media_data['utm_campaign']."' AND utm_content='".$media_data['utm_content']."' AND mb_gubun='PC'";
			$pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
			$mobile_query	= "SELECT * FROM member_info WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' AND utm_source='".$media_data['utm_source']."' AND utm_medium='".$media_data['utm_medium']."' AND utm_campaign='".$media_data['utm_campaign']."' AND utm_content='".$media_data['utm_content']."' AND mb_gubun='MOBILE'";
			$mobile_count	= mysqli_num_rows(mysqli_query($my_db, $mobile_query));
			$unique_query	= "SELECT * FROM member_info WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' AND utm_source='".$media_data['utm_source']."' AND utm_medium='".$media_data['utm_medium']."' AND utm_campaign='".$media_data['utm_campaign']."' AND utm_content='".$media_data['utm_content']."' GROUP BY mb_phone";
			$unique_count	= mysqli_num_rows(mysqli_query($my_db, $unique_query));
?>
                  <tr>
<?
...
?>										
                    <td><?=$media_data["utm_source"]?></td>
                    <td><?=$media_data["utm_medium"]?></td>
                    <td><?=$media_data["utm_campaign"]?></td>
                    <td><?=$media_data["utm_content"]?></td>
